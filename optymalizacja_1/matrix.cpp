...
matrix::matrix(int nv, int mv, double L = 0.0)
{
	if (nv <= 0 || mv <= 0)
...
double& matrix::operator()(int nv = 0, int mv = 0)
{
...
double matrix::operator()(int nv = 0, int mv = 0) const
{
	if (nv >= n || mv >= m || nv < 0 || mv < 0)
...
void matrix::add_col(double L = 0.0)
{
	matrix A(n, m + 1);
	for (int i = 0; i < n; ++i)
	{
...
void matrix::add_row(double L = 0.0)
{
	matrix A(n + 1, m);
	for (int i = 0; i < n; ++i)
		for (int j = 0; j < m; ++j)
