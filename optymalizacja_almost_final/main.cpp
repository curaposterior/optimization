...
{	
	...
	//expansion
	/*
	X0=100, d=1.0, alpha=2.0 Nmax=1000 [36, 84]
	X0= - 100, d=1.0, alpha=2.0 Nmax=1000 [-36, 156]
	X0=10, d=1.0, alpha=1.5 Nmax=1000 [-7.08594 ,    2.40625]
	*/
	/*
	double* borders = expansion(funkcja_celu_lab_2, 10, 1.0, 1.5, 1000);
	cout << "borders (" << borders[0] << ", " << borders[1] << ")\n";*/


	/*
	1. 	opt = fib(ff1T, -10, 10, 1e-5);
	x = -2,10655e-12;
	y = -7,44939e-18;
	f_calls = 62
	Exit flag: 0
	...
	*/
	...
	//opt = fib(funkcja_celu_lab_2, -10, 10, 1e-5);
	//opt = fib(funkcja_celu_lab_2, 10, 100, 1e-5);
	

	//lagrange
	/*
	1.	opt = lag(ff1T, -100, 100, 0.001, 1e-7, 1000);
	x = 2,3348e-13;
	y = -7,44939e-18;
	f_calls = 6
	Exit flag: 

	2.	opt = lag(ff1T, -10, 1, 0.0001, 1e-7, 1000);
	x = 3,82139e-13;
	y = -7,44939e-18;
	f_calls = 5
	Exit flag: 0
	*/
	opt = lag(funkcja_celu_lab_2, -100, 100, 0.001, 1e-7, 1000);
	//opt = lag(funkcja_celu_lab_2, -10, 1, 0.0001, 1e-7, 1000);
