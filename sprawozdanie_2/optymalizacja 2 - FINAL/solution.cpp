...
void solution::my_pick()
{
	string buffer = "";
	
	buffer += " " + std::to_string(m2d(x(0))) + " " + std::to_string(m2d(x(1))) + " " + std::to_string(m2d(y)) + " " + std::to_string(solution::f_calls);
	if (-0.001 < y && y < 0.001) buffer += " tak";
	else buffer += " nie";
...
void my_picky(const solution& A)
{
	...
	buffer += " " + std::to_string(m2d(A.x(0, 0))) + " " + std::to_string(m2d(A.x(1, 0))) + " " + std::to_string(m2d(A.y)) + " " + std::to_string(solution::f_calls);
	if (A.y > -0.0000001) buffer += " lokalne";
	else buffer += " globalne";
