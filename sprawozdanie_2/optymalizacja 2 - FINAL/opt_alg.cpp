...
solution HJ_trial(matrix(*ff)(matrix, matrix, matrix), solution XB, double s, matrix ud1, matrix ud2)
{
	...
	{
		int n = get_dim(XB);
		matrix e_mat = ident_mat(n);
		for (int j = 0; j < n; j++)
		{
			solution x_a = XB.x + s * e_mat[j];
			x_a.fit_fun(ff, ud1, ud2);
			if (x_a.y < XB.y)
				XB = x_a;
			else
			{
...
solution Rosen(matrix(*ff)(matrix, matrix, matrix), matrix x0, matrix s0, double alpha, double beta, double epsilon, int Nmax, matrix ud1, matrix ud2)
{
	...
	{
		solution Xopt;
		const int n = get_dim(x0);
		matrix d_vecs = ident_mat(n);
		matrix lambda = matrix(n, 1);
		matrix p_vec = matrix(n, 1);
		solution xB = x0;
		...
		while (true)
		{
			...
			for (int j = 0; j < n; j++)
			{
				solution x_a = xB.x + s_vec(j) * d_vecs[j];
				x_a.fit_fun(ff, ud1, ud2);
				if (x_a.y < xB.y)
				{
					xB = x_a;
					lambda(j) = lambda(j) + s_vec(j);
					s_vec(j) = alpha * s_vec(j);
				}
				else
				{
					s_vec(j) = -beta * s_vec(j);
					p_vec(j) = p_vec(j) + 1;
				}
			}
			bool flag = true;
			for (int j = 0; j < n; j++)
				if (lambda(j) == 0 || p_vec(j) == 0)
				{
					flag = false;
					break;
				}
			if (flag)
			{
				matrix D_vecs = d_vecs;
				matrix Q_mat = matrix(n, n);
				for (int j = 0; j < n; j++)
					for (int k = 0; k < j + 1; k++)
						Q_mat(j, k) = lambda(j);
				Q_mat = D_vecs * Q_mat;
				for (int j = 0; j < n; j++)
				{
					...
					{
						matrix d_k = d_vecs[k];
						v_vec = v_vec - trans(Q_j) * d_k * d_k;
					}
					d_vecs.set_col(v_vec, j);
				}
				lambda = matrix(n, 1);
				p_vec = matrix(n, 1);
				s_vec = s0;
