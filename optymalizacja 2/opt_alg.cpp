...
solution Rosen(matrix(*ff)(matrix, matrix, matrix), matrix x0, matrix s0, double alpha, double beta, double epsilon, int Nmax, matrix ud1, matrix ud2)
{
	...
	try
	{
		...
		solution* d = new solution[Nmax];		d[0] = e1;
		solution* lambda = new solution[Nmax];	lambda[0] = 0;
		solution * p = new solution[Nmax];		p[0] = 0;
		solution* s = new solution[Nmax];		s[0] = s0;
		solution xb = x0;		
		...
		while (true)
		{
			...
			bool value = true;
			for (int _i_ = 0; _i_ < Nmax; _i_++)
			{
				if (!(lambda[_i_].x != 0 && p[_i_].x != 0))
				{
					value = false;
					break;
				}
